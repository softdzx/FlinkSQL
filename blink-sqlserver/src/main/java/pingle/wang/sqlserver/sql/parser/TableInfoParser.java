package pingle.wang.sqlserver.sql.parser;

import pingle.wang.util.SqlUtil;
import pingle.wang.util.StringUtil;
    ...
    public static TableInfo parser(String ddl){
        ...
        for (String fieldInfo: fieldInfos){
            ...
            if (virtual_matcher.find()){
                ...
                // cast(feeds as string)在sql实际是cast(feeds as varchar)
                if (expression.contains("integer")){
                    expression = expression.replaceAll("integer","int");
                }
                if (expression.contains("long")){
                    expression = expression.replaceAll("long","bigint");
                }
                if (expression.contains("string")){
                    expression = expression.replaceAll("string","varchar");
                }
